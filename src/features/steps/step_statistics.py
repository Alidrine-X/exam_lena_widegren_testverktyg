...
@then(u'ska texten "Våra användare har hjärtmarkerat {amount:d} böcker." visas')
def step_amount_of_favorite_books_in_list(context, amount):
    favorite_list_count = context.page.get_by_test_id("stars-count")
    expect(favorite_list_count).to_contain_text(f"Våra användare har hjärtmarkerat {amount} böcker.")
